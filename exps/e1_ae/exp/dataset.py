...
class TestPredDataset(Dataset):
    def __init__(self, ref_data_path, cor_data_path, sag_data_path, labelcol, seed):
        ...
        for i, gene in enumerate(genes_unique):
            ...
            # If the gene is unique in the coronal set, just use that one
            else:
    ...
    def __getitem__(self, idx):
        pred = self.pred_arr[idx]
        y = self.target_arr[idx]

        return  torch.tensor(pred), torch.tensor(y)
...
class MouseHumanDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        self.train_ds, self.val_ds = train_test_split(full_ds, train_size=0.8)
...
class MouseHumanDataModuleCV(pl.LightningDataModule):
    def __init__(
        self,
        seed: int,
        mouse_voxel_data_path: str,
...
def encode(
    ...
    preds = trainer.predict(dataloaders=dataloader, model=model, ckpt_path=ckpt_path)
...
def autoencode(
    ...
    preds = trainer.predict(model=model, dataloaders=dataloader, ckpt_path=ckpt_path)
